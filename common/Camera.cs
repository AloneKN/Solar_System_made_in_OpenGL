...
namespace MyGame
{
    ...
    public class Camera
    {
        ...
        public static float TurboVel { get; set; } = 10.0f;
        private void KeysUpdatePosition()
        {
            ...
            if(input(Keys.LeftShift) && input(Keys.W)) Position += Front * MathF.Pow(cameraSpeed, TurboVel) * (float)Clock.ElapsedTime;
            if(input(Keys.LeftShift) && input(Keys.S)) Position -= Front * MathF.Pow(cameraSpeed, TurboVel) * (float)Clock.ElapsedTime;
